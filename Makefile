...
CANOW_DIR := $(CURDIR)/cmd/canow-chain
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=canow-chain \
	-X github.com/cosmos/cosmos-sdk/version.AppName=canow-chain \
...
install: go.sum go-version
	go install -mod=readonly $(BUILD_FLAGS) $(CANOW_DIR)

build: go.sum go-version
	@mkdir -p $(BUILD_DIR)
	@echo $(BUILD_FLAGS)
	go build -mod=readonly -o $(BUILD_DIR) $(BUILD_FLAGS) $(CANOW_DIR)
